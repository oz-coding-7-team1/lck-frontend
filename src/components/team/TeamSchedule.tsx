...
async function getTeamEvents(teamId: number): Promise<any[]> {
  try {
      ...
      {teamEvents.length > 0 ? (
        <ul className="mt-4 space-y-4">
          {teamEvents.map(async (event) => {
            ...
            let opponentName =
              parts.find((name: string) => name !== team.name) ||
              "상대 팀 없음";
            ...
            try {
              ...
              opponentTeam = teamResponse.data.find(
            ...
            const formattedDate = event.start
              ...
              : "날짜 정보 없음";
            const formattedTime = event.start
        ...
        <p className="mt-3 text-gray-500">현재 등록된 경기 일정이 없습니다.</p>
      )}
    </>
  );
}
