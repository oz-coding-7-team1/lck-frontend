...
export default async function PlayerPage({ params }: Props) {
  ...
  const playerSubscribeCount = await subscriptionApi.getPlayerSubscriptionCount(
    player.id
  );
          ...
          <div className="col-span-3 p-4 border rounded-lg shadow">
            ...
            <p className="py-2">
              ...
              <strong className="block text-xl">
                {playerDetails.data.realname}
              </strong>
            </p>
            <p className="py-2">
              생년월일
              <strong className="block text-xl">
                {playerDetails.data.date_of_birth}
              </strong>
            </p>
            <p className="py-2">
              국적
              <strong className="block text-xl">
                {playerDetails.data.nationality}
              </strong>
            </p>
            <p className="py-2">
              데뷔
              <strong className="block text-xl">
                {playerDetails.data.debut_date}
              </strong>
            </p>
            <p className="py-2">
              포지션
              <strong className="block text-xl">
                {playerDetails.data.position}
              </strong>
            </p>
            <p className="py-2">
              소속 팀
              <strong className="block text-xl">
                {team ? team.name : "소속팀 없음"}
              </strong>
            </p>
            <p className="py-2">
              소속사
              <strong className="block text-xl">
                {playerDetails.data.agency}
              </strong>
            </p>
            <p className="py-2">
              game ID
              <strong className="block text-xl">
                {playerDetails.data.gamename}
              </strong>
            </p>
          </div>
